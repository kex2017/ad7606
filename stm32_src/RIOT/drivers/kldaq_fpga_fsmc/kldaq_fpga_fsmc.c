...
static int kldaq_fpga_fsmc_set_changerate(kldaq_dev_t *daq, uint8_t channel, uint16_t change_rate);
...
static int kldaq_fpga_fsmc_get_pd_curve_time(kldaq_dev_t *daq,uint32_t *time);
...
const kldaq_func_t kldaq_fpga_fsmc_driver = {
    ...
    .set_changerate                 = kldaq_fpga_fsmc_set_changerate,
	...
	.get_pd_curve_time              = kldaq_fpga_fsmc_get_pd_curve_time,
...
static int kldaq_fpga_fsmc_set_fpga_work_mode(kldaq_dev_t *daq, uint8_t mode)
{
    ...
    //设置FPGA工作在主动模式，捕获到越限或变化率越限保存原始波形。
    if (mode == ACTIVE_MODE) {
        value = fpga_read(15);
        value &= 0xfffe;
        fpga_write(15, value);
    }
    //设置 FPGA工作在被动模式，FPGA在收到采集命令(reg8的bit0为1)则无条件采集一段局放原始波形。
    else if (mode == NEGATIVE_MODE) {
        value = fpga_read(15);
        value |= 1;
        fpga_write(15, value);
    }
