...
static void init_g_kl_twiddleCoef_1024(void);
static void init_g_kl_armBitRevIndexTable1024(void);

#define PI2              (6.28318530717959)
#define SAMPLES          (2048)
#define FFT_SIZE         (SAMPLES/2)
#define Fs               (FFT_SIZE * 50)
#define PERIODS_COUNT    (10)
#define DIELOSS_CHANNEL_CNT  (6) /*4,5,6,7,8,9*/
#define DIELOSS_BASE_CHANNEL (4) /*A1*/
#define MAX_HARMONICS    (13)

float32_t g_kl_twiddleCoef_1024[2048] __attribute__((section(".big_data")));
uint16_t g_kl_armBitRevIndexTable1024[ARMBITREVINDEXTABLE1024_TABLE_LENGTH] __attribute__((section(".big_data")));


const arm_cfft_instance_f32 arm_cfft_sR_f32_len1024 = {
   FFT_SIZE,
   g_kl_twiddleCoef_1024,
   g_kl_armBitRevIndexTable1024,
   ARMBITREVINDEXTABLE1024_TABLE_LENGTH
};

static float32_t lBufInArray_normalize[FFT_SIZE] __attribute__((section(".big_data")));         /* Complex input vector */
static float32_t fft_calc_data[SAMPLES] __attribute__((section(".big_data")));  //[in/out]
static float32_t fft_harmonics_amp_output[FFT_SIZE] __attribute__((section(".big_data")));
static float32_t g_singel_channel_base_phase[PERIODS_COUNT] = {0}; //saved PERIODS_COUNT base phase, used to calc median phase


static channel_harmonics_info_t g_channels_harmonics_info[DIELOSS_CHANNEL_CNT] = {0};

void init_harmonics_info(void)
{
   memset(g_channels_harmonics_info, 0x0, sizeof(g_channels_harmonics_info));
}

channel_harmonics_info_t* get_harmonics_info_by_channel(uint8_t channel)
{
   return &g_channels_harmonics_info[channel - DIELOSS_BASE_CHANNEL];
}

void set_harmonics_info_by_channel(uint8_t channel, float32_t *fft_output, float32_t *harmonics_output, int size)
{
   int i = 0;
   channel_harmonics_info_t *harmonics_info;
   float32_t real, imaginary, amp, phase, base;

   harmonics_info = get_harmonics_info_by_channel(channel);
   base = fft_harmonics_amp_output[1] * 2 / FFT_SIZE;
   LOG_INFO("   i, freq, prop,          amp,    phase,        X,        Y");
   for (i = 0; i < MAX_HARMONICS && i < size; i++) {
      real = fft_output[2 * i];
      imaginary = fft_output[2 * i + 1];
      amp = harmonics_output[i] * 2 / FFT_SIZE;
      harmonics_info->harmonics[i] = amp;
      phase = atan(imaginary / real) * 360.0 / PI2;
      harmonics_info->base_phase = phase;
      LOG_INFO("%4d, %4d, %4.2f, %12.8f, %8.4f, %8.4f, %8.4f", i, Fs / FFT_SIZE * i, amp / base, amp, phase, real, imaginary);
   }
}

float32_t calc_channel_base_phase_diff(uint8_t channel1, uint8_t channel2)
{
    channel_harmonics_info_t *harmonics_info1, *harmonics_info2;

    harmonics_info1 = get_harmonics_info_by_channel(channel1);
    harmonics_info2 = get_harmonics_info_by_channel(channel2);

    return harmonics_info1->base_phase - harmonics_info2->base_phase;
}
...
void show_last_sampled_harmonics_info(uint8_t channel)
{
   ...
   harmonics_info = get_harmonics_info_by_channel(channel);
   base = harmonics_info->harmonics[1];
   LOG_INFO("Get last sampled harmonics info as:");
   LOG_INFO("base_current: %8.4f, base_phase: %8.4f", harmonics_info->base_current, harmonics_info->base_phase);
   LOG_INFO("   i, freq, prop,          amp");
   for (i = 0; i < MAX_HARMONICS; i++) {//i==0 stands for DC component, so proportion should be 0
      LOG_INFO("%4d, %4d, %4.2f, %12.8f", i, Fs / FFT_SIZE * i, i == 0 ? 0 : harmonics_info->harmonics[i] / base, harmonics_info->harmonics[i]);
   }
   if (4 <= channel && channel <= 6) {
       LOG_INFO("harmonics phase: A1-B1: %8.4f, A1-C1: %8.4f, B1-C1: %8.4f", calc_channel_base_phase_diff(4, 5),
                calc_channel_base_phase_diff(4, 6), calc_channel_base_phase_diff(5, 6));
   } else if (7 <= channel && channel <= 9) {
       LOG_INFO("harmonics phase: A2-B2: %8.4f, A2-C2: %8.4f, B2-C2: %8.4f", calc_channel_base_phase_diff(7, 8),
                       calc_channel_base_phase_diff(7, 9), calc_channel_base_phase_diff(8, 9));
   } else {
       LOG_WARN("Invalid dieloss channel: %d, out of range[4~9] ", channel);
   }
}


void show_harmonics_info_with_original_data(float32_t *fft_output, float32_t *harmonics_output, int size)
{
   int i;
   float32_t real, imaginary, amp, phase, base;

   base = fft_harmonics_amp_output[1];
   LOG_DEBUG("   i, freq, prop,          amp,    phase,        X,        Y");
   for (i = 0; i < MAX_HARMONICS && i < size; i++) {
      real = fft_output[2 * i];
      imaginary = fft_output[2 * i + 1];
      amp = harmonics_output[i];
      phase = atan(imaginary / real) * 360.0 / PI2;
      LOG_DEBUG("%4d, %4d, %4.2f, %16.8f, %8.4f, %8.4f, %8.4f", i, Fs / FFT_SIZE * i, amp / base, amp * 2 / FFT_SIZE, phase, real, imaginary);
   }
}

//value in FPGA is uint16_t, but lBufMagArray type is long, so define value type as long too
float convert_ad_to_current(long value, uint8_t channel)
{
   uint8_t base = 6;
   float amplitude;
   ...
   //1000 变比
   //10 电阻大小
   base = channel > 6 ? 96 : 6; //放大倍数
   amplitude = (value * 2.048) / (32768 * base * 10) * 1000;

   return amplitude;
}

void calc_median_phase_by_channel(uint8_t channel)
{
   float32_t median_phase = 0;
   channel_harmonics_info_t *harmonics_info;

   harmonics_info = get_harmonics_info_by_channel(channel);

   arm_mean_f32(g_singel_channel_base_phase, PERIODS_COUNT, &median_phase);
   harmonics_info->base_phase = median_phase;

   LOG_INFO("Get base wave channel median phase: %8.4f", median_phase);
}

void calc_dieloss_harmonics_with_float(int16_t *data, uint8_t index, uint8_t channel)
{
   int i;
   ...
   for (i = 0; i < FFT_SIZE; i++) {
      lBufInArray_normalize[i] = convert_ad_to_current(data[i], channel);
   }
   arm_mean_f32(lBufInArray_normalize, FFT_SIZE, &mean_value);

   for (i = 0; i < FFT_SIZE; i++) {
      fft_calc_data[2 * i] = lBufInArray_normalize[i] - mean_value;
      fft_calc_data[2 * i + 1] = 0.0;
   }

   arm_cfft_f32(&arm_cfft_sR_f32_len1024, fft_calc_data, ifftFlag, doBitReverse);
   arm_cmplx_mag_f32(fft_calc_data, fft_harmonics_amp_output, FFT_SIZE);

   g_singel_channel_base_phase[index] = atan(fft_calc_data[3] / fft_calc_data[2]) * 360.0 / PI2;;
//   if (index == 0) {
//      set_harmonics_info_by_channel(channel, fft_calc_data, fft_harmonics_amp_output, FFT_SIZE);
//   }
//   show_harmonics_info_with_original_data(fft_calc_data, fft_harmonics_amp_output, FFT_SIZE);
}

/*****************************************************************************
 * BE CAREFUL
 * ensure call init_cfft_config() first if you want to calculate harmonics
 * ***************************************************************************/
void init_cfft_config(void)
{
    init_g_kl_twiddleCoef_1024();
    init_g_kl_armBitRevIndexTable1024();
}

void init_cfft_config_if_need(void)
{
...
void calc_dieloss_harmonics(int16_t *data, uint8_t channel)
{
   ...
   init_cfft_config_if_need();

   if (channel <4 || channel > 9) {
       return;
   }
   for (i = 0; i < PERIODS_COUNT; i++) {
      LOG_INFO("Calculate dieloss harmonics: channel:%d, periods:%d", channel, i);
      calc_dieloss_harmonics_with_float(data + i * FFT_SIZE, i, channel);
   }

   calc_median_phase_by_channel(channel);
}

void arm_cfft_f32_app_test(void)
{
   int fs = 10240;
   int i;
   float32_t mean_value;
   ...
   for (i = 0; i < FFT_SIZE; i++) {
      lBufInArray_normalize[i] = 1+ 1*arm_sin_f32(2*PI*10*i/fs)+ 3*arm_sin_f32(2*PI*40*i/fs)+ 5*arm_sin_f32(2*PI*80*i/fs);
   }
   arm_mean_f32(lBufInArray_normalize, FFT_SIZE, &mean_value);
   ...
   arm_cfft_f32(&arm_cfft_sR_f32_len1024, fft_calc_data, ifftFlag, doBitReverse);
   arm_cmplx_mag_f32(fft_calc_data, fft_harmonics_amp_output, FFT_SIZE);
//   arm_max_f32(testOutput, FFT_SIZE, &max_value, &testIndex);
...
static void init_g_kl_twiddleCoef_1024(void)
{
    float32_t local_twiddleCoef_1024[2048] = {
    ...
    memcpy((void*)g_kl_twiddleCoef_1024, (void*)local_twiddleCoef_1024, sizeof(g_kl_twiddleCoef_1024));
}

static void init_g_kl_armBitRevIndexTable1024(void)
{
    uint16_t local_armBitRevIndexTable1024[ARMBITREVINDEXTABLE1024_TABLE_LENGTH] = {
                    //8x2, size 1800
    ...
    memcpy((void*)g_kl_armBitRevIndexTable1024, (void*)local_armBitRevIndexTable1024, sizeof(g_kl_armBitRevIndexTable1024));
}
