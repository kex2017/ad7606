...
static int kldaq_fpga_spi_write_register(kldaq_dev_t *daq, uint8_t channel, uint16_t reg, uint16_t value);
static uint16_t kldaq_fpga_spi_read_register(kldaq_dev_t *daq, uint8_t channel, uint16_t reg);
...
static int kldaq_fpga_spi_enable_order_read(kldaq_dev_t *daq, uint8_t channel);

static int kldaq_fpga_spi_read_cycle_num(kldaq_dev_t *daq, uint8_t channel);
static int kldaq_fpga_spi_write_cycle_num(kldaq_dev_t *daq, uint8_t channel, uint16_t cycle_num);
static int kldaq_fpga_spi_stop_sample(kldaq_dev_t *daq, uint8_t channel);
static int kldaq_fpga_spi_enable_entire_read(kldaq_dev_t *daq, uint8_t channel);
static int kldaq_fpga_spi_read_entire_data(kldaq_dev_t *daq, uint8_t channel);

static int kldaq_fpga_spi_clear_sample_flag(kldaq_dev_t *daq, uint8_t channel);
static int kldaq_fpga_spi_is_sample_done(kldaq_dev_t *daq, uint8_t channel);
...
const kldaq_func_t kldaq_fpga_spi_driver = {
    ...
    .read_reg          = kldaq_fpga_spi_read_register,
    .write_reg         = kldaq_fpga_spi_write_register,
	...
	.is_sample_done    = kldaq_fpga_spi_is_sample_done,
	.clear_sample_flag = kldaq_fpga_spi_clear_sample_flag,
	.enable_order_read = kldaq_fpga_spi_enable_order_read,
	.read_cycle_num    = kldaq_fpga_spi_read_cycle_num,
	.write_cycle_num   = kldaq_fpga_spi_write_cycle_num,
	.stop_sample       = kldaq_fpga_spi_stop_sample,
	.enable_entire_read= kldaq_fpga_spi_enable_entire_read,
	.read_entire_data  = kldaq_fpga_spi_read_entire_data,
};

/* fpga spi pin cfg */
#define KLSTM32_FPGA_SPI      SPI_DEV(1)
#define KLSTM32_FPGA_SPI_CS   GPIO_PIN(PORT_B, 5)
...
static uint16_t kldaq_fpga_spi_read_register(kldaq_dev_t *daq, uint8_t channel, uint16_t reg)
{
    ...
    _spi_read_register(daq, channel, reg, &value);
...
static int kldaq_fpga_spi_write_register(kldaq_dev_t *daq, uint8_t channel, uint16_t reg, uint16_t value)
{
    _spi_write_register(daq, channel, reg, value);
    return 0;
}
...
static int kldaq_fpga_spi_clear_sample_flag(kldaq_dev_t *daq, uint8_t channel)
{
   _spi_write_register(daq, channel, 1, 1);
...
static int kldaq_fpga_spi_is_sample_done(kldaq_dev_t *daq, uint8_t channel)
{
   uint16_t ret = 0;
   _spi_read_register(daq, channel, 1, &ret);
   return ret & CHA_STA_REG_OVER_MASK;
}

static int kldaq_fpga_spi_enable_order_read(kldaq_dev_t *daq, uint8_t channel)
{
   _spi_write_register(daq, channel, 3, 1);
...
{
    uint16_t ret = 0;
    _spi_read_register(daq, channel, 0, &ret);
    return ret & CHA_STA_REG_CYCLE_NUM_MASK;
}

static int kldaq_fpga_spi_write_cycle_num(kldaq_dev_t *daq, uint8_t channel, uint16_t cycle_num)
{
   _spi_write_register(daq, channel, 7, cycle_num);
...
static int kldaq_fpga_spi_stop_sample(kldaq_dev_t *daq, uint8_t channel)
{
   _spi_write_register(daq, channel, 4, 1);
...
static int kldaq_fpga_spi_enable_entire_read(kldaq_dev_t *daq, uint8_t channel)
{
   _spi_write_register(daq, channel, 5, 1);
...
static int kldaq_fpga_spi_read_entire_data(kldaq_dev_t *daq, uint8_t channel)
{
   _spi_write_register(daq, channel, 6, 1);
...
//   uint16_t reg0 = (REG0_CHANNEL_STARTBIT << channel);
   ...
   _spi_write_register(daq, channel, 8, 0x0001);
