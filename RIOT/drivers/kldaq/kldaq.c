...
int kldaq_init(kldaq_dev_t *daq)
{
   ...
   if (daq->cnf->init && daq->driver->init) {
      if ((ret = daq->cnf->init(daq->cnf)) != 0) {
         LOG_WARN("DAQ Config Init Failed");
         return ret;
      }

      if ((ret = daq->cnf->download_image(daq->cnf)) != 0) {
         LOG_WARN("DAQ Download Image Failed");
         return ret;
      }

      if ((ret = daq->driver->init(daq)) != 0) {
         LOG_WARN("DAQ Function Init Failed");
         return ret;
      }
   }
   return -ENOTSUP;
...
int kldaq_write_reg(kldaq_dev_t *daq, uint16_t reg, uint16_t value)
{
	...
	if (daq->driver->write_reg) {
		return daq->driver->write_reg(daq, reg, value);
	} else {
		return -ENOTSUP;
	}
}
...
uint16_t kldaq_read_reg(kldaq_dev_t *daq, uint16_t reg)
{
	...
	if (daq->driver->read_reg) {
		return daq->driver->read_reg(daq, reg);
	} else {
		return -ENOTSUP;
	}
}
