...
//        gpio_set(soft_spi_config[bus].miso_pin);
    }
}
...
uint8_t soft_spi_readin_byte(soft_spi_t bus, soft_spi_cs_t cs, bool cont, uint8_t* in)
{
                        ...
                        const void *out, void *in, size_t len)
{
    ...
    if (NULL != in) {
        for (size_t i = 0; i < len - 1; i++) {
            uint8_t retval = soft_spi_readin_byte(bus, cs, true, &tmp);
            if (NULL != in) {
                ((uint8_t *)in)[i] = retval;
            }
        }

        soft_spi_readin_byte(bus, cs, cont, &tmp);
        if (NULL != in) {
