#include "temp.h"
#include "periph/uart.h"
#include "isrpipe.h"
#include "x_queue.h"

const uint8_t auchCRCHi[] = {  };

const uint8_t auchCRCLo[] = {  };

#define UART_TEMP_DEV           UART_DEV(4)
#define UART_TEMP_BAUDRATE      (9600)

#define TEMP_RX_BUFSIZE    (32)
static char _rx_buf_mem[TEMP_RX_BUFSIZE];
isrpipe_t uart_temp_isrpipe = ISRPIPE_INIT(_rx_buf_mem);

int temp_init(void) //FIXME:串口中断接收数据可能丢
{
    return uart_init(UART_TEMP_DEV, UART_TEMP_BAUDRATE, (uart_rx_cb_t)isrpipe_write_one, &uart_temp_isrpipe);
}

static void add_data_to_queue(circular_queue_t *data_queue, char *buf, int len)
{
    int index = 0;

    if (len > 0) {
        for (index = 0; index < len; index++) {
            add_queue(data_queue, buf[index]);
        }
    }
}

int temp_read(circular_queue_t* data_queue)
{
    int ret = 0;
    char buf[32] = { 0 };
    ret = isrpipe_read_timeout(&uart_temp_isrpipe, buf, sizeof(buf), 10);
    add_data_to_queue(data_queue, buf, ret);
    return ret;
}

void temp_send(const uint8_t *data, uint16_t len)
{
    uart_write(UART_TEMP_DEV, data, (size_t)len);
}

uint16_t temp_crc16_check(uint8_t *puchMsgg, uint8_t usDataLen) //CRC16
{
    uint8_t uchCRCHi = 0xFF;
    uint8_t uchCRCLo = 0xFF;
    unsigned uIndex;

    while (usDataLen--) {
        uIndex = uchCRCHi ^ *puchMsgg++;
        uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex];
        uchCRCLo = auchCRCLo[uIndex];
    }
    return (uchCRCHi * 256 + uchCRCLo);
}
