...
//extra operation code for SPI NOR flash opcode table: mtd_spi_nor_opcode_t
#define OPCODE_WRDI 0x04  /* Write-Disable */
#define OPCODE_EWSR 0x50  /* Write-Disable */
#define OPCODE_AAI_WORD_PROGRAM 0xad  /* Auto Address Increment Programming */
...
/**
 * @internal
 * @brief    Continue to write data to flash with AAI mode
 * */
static void mtd_spi_cmd_continue_aai(const mtd_spi_nor_t *dev, void *src)
{
    spi_acquire(dev->spi, dev->cs, dev->mode, dev->clk);
    spi_transfer_byte(dev->spi, dev->cs, true, OPCODE_AAI_WORD_PROGRAM);
    spi_transfer_bytes(dev->spi, dev->cs, false, (void *)src, NULL, WORD_SIZE);
    spi_release(dev->spi);
}

/**
 * @internal
 * @brief    First to write data to flash with AAI mode
 * */
void mtd_spi_cmd_fisrt_aai(const mtd_spi_nor_t* dev, const void* src, uint8_t* addr_buf)
{
    /* Acquire exclusive access to the bus. */
    spi_acquire(dev->spi, dev->cs, dev->mode, dev->clk);
    spi_transfer_byte(dev->spi, dev->cs, true, OPCODE_AAI_WORD_PROGRAM);
    ...
    be_uint32_t addr, const void* src, uint32_t count)
{
    ...
    if (count <= 1) {
        TRACE("Byte write or section erase write.\n");
        /* Acquire exclusive access to the bus. */
        spi_acquire(dev->spi, dev->cs, dev->mode, dev->clk);
        do {
            /* Send opcode followed by address */
        ...
        void *data = (void*)src;
        /* Acquire exclusive access to the bus. */
        mtd_spi_cmd_fisrt_aai(dev, data, addr_buf);
        data += WORD_SIZE;
        count -= WORD_SIZE;
        wait_for_write_complete(dev);

        while (count > 0) {
            TRACE("Continue AAI write.\n");
            mtd_spi_cmd_continue_aai(dev, data);
            data += WORD_SIZE;
            count -= WORD_SIZE;
            wait_for_write_complete(dev);
        }

        mtd_spi_cmd(dev, OPCODE_WRDI);
    }
}
...
/**
 * @internal
 * @brief    enable write status register before write
 **/
static void mtd_spi_enable_write_status_register(const mtd_spi_nor_t *dev)
{
    spi_acquire(dev->spi, dev->cs, dev->mode, dev->clk);

    spi_transfer_byte(dev->spi, dev->cs, false, OPCODE_EWSR);//ewsr
    spi_transfer_byte(dev->spi, dev->cs, true, dev->opcode->wrsr);
    spi_transfer_byte(dev->spi, dev->cs, true, 0x00); //TODO: magic number for register status value, and value should be different
...
static int mtd_spi_nor_write(mtd_dev_t *mtd, const void *src, uint32_t addr, uint32_t size)
{
    ...
    /* write enable */
    mtd_spi_enable_write_status_register(dev);
