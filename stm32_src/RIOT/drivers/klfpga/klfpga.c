...
const uint32_t g_fpga_image_len = 272384;//163840


uint8_t READ_FPGA_CFG_nSTATUS(void)
{
    return !!gpio_read(FPGA_nSTS_PIN);
}

uint8_t READ_FPGA_CFG_CONF_DONE(void)
{
    return !!gpio_read(FPGA_DONE_PIN);
}
...
static int kldaq_fpga_ps_power(const klfpga_conf_t *fpga, enum klfpga_power_state power)
{
    ...
    if (power == KLFPGA_POWER_UP) {
//        ext_pm_power_on_jf();
    }
    else if (power == KLFPGA_POWER_DOWN) {
//        ext_pm_power_off_jf(); // add by Xiewei, power off 1 seconds, and then power on
    }
    else {
//        LOG_WARN("Invalid operation for FPGA power");
    }
