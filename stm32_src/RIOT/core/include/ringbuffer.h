...
/**
 * @brief           return ringbuffer alive.
 * @param[in,out]   rb    Ringbuffer to operate on.
 * @returns         number of avail bytes
 */
static inline unsigned int ringbuffer_avail(const ringbuffer_t *__restrict rb)
{
    return rb->avail;
}
...
/**
 * @brief           Read, but don't remove, the index element in the buffer.
 * @param[in]       rb    Ringbuffer to operate on.
 * @returns         the index value
 */
int ringbuffer_peek_index(const ringbuffer_t *__restrict rb,unsigned index);
